...
result = subprocess.run(
    ["cosmic-ray", "dump", "cosmic-ray.sqlite"], capture_output=True, text=True
)
...
print(
    f"\n✅ Killed (テストが検出):     {killed:3d}  ({killed/total*100:.1f}%)"
    if total > 0
    else ""
)
print(
    f"❌ Survived (テストが未検出):  {survived:3d}  ({survived/total*100:.1f}%)"
    if total > 0
    else ""
)
print(
    f"💀 Incompetent (コード破損):   {incompetent:3d}  ({incompetent/total*100:.1f}%)"
    if total > 0
    else ""
)
if timeout > 0:
